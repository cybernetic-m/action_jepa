import torch
import glob
import os
import numpy as np
from torch.utils.data import Dataset

class PredictorDataset(Dataset):
    def __init__(self, data_dir, selected_tasks, task_ids, preprocess_fn = None):

        super(PredictorDataset, self).__init__()

        self.file_paths = []   # a list of all the file paths (Ex. ./resampled_data/libero_10/task_0_demo_0.pt)
        self.preprocess_fn = preprocess_fn
        self.selected_tasks = selected_tasks
        self.task_ids = task_ids
        self.data_dir = data_dir
        
        for task in self.selected_tasks:
            for task_id in self.task_ids:
                task_path = os.path.join(self.data_dir, task, str(task_id), 'data', "*.pt") # the generic path of the type "./processed_data/libero_10/*.pt"
                # with glob.glob we create a list of path ['.../task_0_demo_0', '.../task_0_demo_1', ...], one list for all the tasks
                # with extend we do not append list of lists but create a unique list for all the tasks paths
                files_found = sorted(glob.glob(task_path))
                self.file_paths.extend(files_found)
        
        self.file_paths.sort()
        self.window_indices = []
        self.demos = []

        for data_idx, path in enumerate(self.file_paths):
            data = torch.load(path, map_location='cpu', weights_only=False)

            self.demos.append(data)
            steps = data['frames'].shape[0] 
            
            for start_idx in range(0, steps-4):
                self.window_indices.append((data_idx,start_idx))
        self.window_indices.sort()
            

    def __len__(self):
        return len(self.window_indices) 
    
    def __getitem__(self, index):
        data_idx, start_idx = self.window_indices[index] 
        demo = self.demos[data_idx]
        #demo_path = self.file_paths[data_idx]

        #demo = torch.load(demo_path, map_location='cpu', weights_only=False)

        frames = demo['frames']
        frames_current = frames[start_idx:start_idx+2]
        frames_next = frames[start_idx+2:start_idx+4]

        if self.preprocess_fn is not None:
            frames_current = self.preprocess_fn(frames_current, device='cpu').squeeze(0)
            frames_next = self.preprocess_fn(frames_next, device='cpu').squeeze(0)
        
        actions = demo['actions'].float()
        action = actions[start_idx]
    
        return {'frames_current': frames_current,
                'frames_next': frames_next,
                'action': action,
                } 

