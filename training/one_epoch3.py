...
def one_epoch(model, dataloader, optimizer, loss_fn, device, scaler, lambda_actor = 1.0, lambda_refiner = 1.0, lambda_predictor=1.0, validation = False):
    ...
    with grad_modality:
        for batch in pbar:
            ...
            with torch.no_grad():
                z_target = model.preprocess_frames(vision_target)

            with autocast(device_type='cuda', enabled=(device == 'cuda')):
                # Making the predictions
                actor_action_seq_pred, refiner_action_seq_pred, z_pred = model(text_input, vision_input, joint_input)
            ...
            with torch.no_grad():
                ...
                actor_cosim_ori = F.cosine_similarity(actor_pred_ori, target_ori, dim=-1).mean()
