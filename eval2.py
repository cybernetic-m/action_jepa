...
if __name__ == '__main__':
    ...
    global_success = {"actor_refiner": 0}

    # Due dizionari separati per i report JSON
    metrics_report_actor_refiner = {
        "benchmark_name": task_suite_name,
        "model_tested": "ACTOR+REFINER",
    ...
    for task_id in range(n_tasks):
        ...
        task_video_dir_actor_refiner = os.path.join(policy_dir_path, task_folder_name, "actor_refiner_videos")
        os.makedirs(task_video_dir_actor_refiner, exist_ok=True)
    
        task_success_count = {"actor_refiner": 0}
        saved_videos_count = {"actor_refiner": 0}
        ...
        for ep in range(total_episodes_per_task):
            ...
            with torch.no_grad():
                ...
                while step < max_steps:
                    ...
                    action = (actor_action[0] + refiner_action[0])/2
                    chunk_actions = action.cpu().numpy()
                ...
                if is_success:
                    task_success_count["actor_refiner"] += 1
                    global_success["actor_refiner"] += 1
                    print(f"✅ Ep {ep+1:02d} - Success!")

                    if saved_videos_count["actor_refiner"] < MAX_VIDEOS_PER_TASK and len(video_frames) > 0:
                        videoname = f"actor_refiner_success_ep{ep+1:02d}.mp4"
                        imageio.mimsave(os.path.join(task_video_dir_actor_refiner, videoname), video_frames, fps=60)
                        saved_videos_count["actor_refiner"] += 1
                else:
                    print(f"❌ Ep {ep+1:02d} - Failed.")

        env.close()
        
        sr_actor_refiner = (task_success_count["actor_refiner"] / total_episodes_per_task) * 100
        
        print(f"\n📊 [Result Task {task_id}] Actor + Refiner SR: {task_success_count['actor_refiner']}/{total_episodes_per_task} ({sr_actor_refiner:.1f}%)")

        metrics_report_actor_refiner["tasks_details"][task_folder_name] = {
            "task_id": task_id,
            "description": task_description,
            "success_count": task_success_count["actor_refiner"],
            "total_episodes": total_episodes_per_task,
            "success_rate_percentage": float(sr_actor_refiner)
        }

total_evaluations = n_tasks * total_episodes_per_task

overall_sr_actor_refiner = (global_success["actor_refiner"] / total_evaluations) * 100
metrics_report_actor_refiner["global_success_count"] = global_success["actor_refiner"]
metrics_report_actor_refiner["total_demos_executed"] = total_evaluations
metrics_report_actor_refiner["overall_success_rate"] = float(overall_sr_actor_refiner)


print("\n" + "="*50)
print(f"🏆 OVERALL ACTOR + REFINER SUCCESS RATE:   {global_success['actor_refiner']}/{total_evaluations} ({overall_sr_actor_refiner:.2f}%)")
print("="*50 + "\n")

json_actor_refiner_path = os.path.join(policy_dir_path, f"{task_suite_name}_actor_refiner_metrics.json")
with open(json_actor_refiner_path, "w", encoding="utf-8") as f:
    json.dump(metrics_report_actor_refiner, f, indent=4, ensure_ascii=False)

print(f"📄 JSON metrics successfully saved to:\n - {json_actor_refiner_path}")
