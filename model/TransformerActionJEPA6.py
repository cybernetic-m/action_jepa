...
class TransformerActionJEPA(nn.Module):
    def __init__(self, 
        ...
        self.aggregation_mode_actor = aggregation_mode_actor
        self.aggregation_mode_refiner = aggregation_mode_refiner
    ...
    def forward(self, language_input, vision_input, joint_input):
        ...
        if self.aggregation_mode_actor == "CMA":
            # ATTENTION TEXT*PATCHES
            attn_logits = torch.bmm(z_obs_proj, z_text_proj.transpose(1, 2)) 
            attn_weights = torch.softmax(attn_logits, dim=1)
            z_obs_context = torch.sum(z_obs_proj * attn_weights, dim=1, keepdim=True) 
        elif self.aggregation_mode_actor == "GAP":
            # GLOBAL AVERAGE POOLING
            z_obs_context = torch.mean(z_obs_proj, dim=1, keepdim=True)
        elif self.aggregation_mode_actor == "MAX":
            # GLOBAL MAX POOLING
        ...
        if self.aggregation_mode_refiner == "CMA":
            ...
            attn_logits_pred = torch.bmm(z_pred_proj, z_text_proj.transpose(1, 2)) 
            attn_weights_pred = torch.softmax(attn_logits_pred, dim=1)
            z_pred_context = torch.sum(z_pred_proj * attn_weights_pred, dim=1, keepdim=True) 
        elif self.aggregation_mode_refiner == "GAP":
            # GLOBAL AVERAGE POOLING
            z_pred_context = torch.mean(z_pred_proj, dim=1, keepdim=True)
        elif self.aggregation_mode_refiner == "MAX":
            # GLOBAL MAX POOLING
